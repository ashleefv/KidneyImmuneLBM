% call_coupledODE_model script to perform the below listed tasks.
% Author: Krutika Patidar
% Description: This call file calls various functions to run coupledODE
...
% Most supporting file names for this model begin with coupledODE_###.m
%%
...
[params, y0] = coupledODE_opt_extended_loadParams(choice); %in vitro as optimized
...
% Download kdefunc to use this functionality
addpath("~/kdefunc") 

% Run coupledODE function with optimal parameter set
% choose mode=0 :no plots
% mode = 1 :plots
% mode = 2 :additional validation plots
load invitro_data.mat
mode = 0;
[Time, Y_pred] = coupledODE_run(tspan, y0, params, mode);
%%
%% Model Workflow
...
[min_error, w_error] = coupledODE_error(global_p_best, params, y0, tspan, tau_index, n_index, k_index);

%% Plot graph of select response variables
% Time: Time output
% Y_pred: Predicted output
% global_p_best: partial list of parameters tuned by fmincon
% params: Complete list of parameters required to run coupledODE model
