...
%
dydt(ActinS) = (act(y(pMLC),rpar(:,46))*ymax(ActinS) - y(ActinS))/tau(ActinS); %switched for readability, does not impact results
...
dydt(VEGFamRNA) = (act(y(NFKB),rpar(:,15))*ymax(VEGFamRNA) - y(VEGFamRNA))/tau(VEGFamRNA); %rpar index fix from dydt(VEGFamRNA) = (act(y(NFKB),rpar(:,17))*ymax(VEGFamRNA) - y(VEGFamRNA))/tau(VEGFamRNA); 
...
dydt(ROSec) = (OR(act(y(NADPHec),rpar(:,24)),act(y(eNOS),rpar(:,33)))*ymax(ROSec) - y(ROSec))/tau(ROSec); %rpar index fix from dydt(ROSec) = (OR(act(y(NADPHec),rpar(:,24)),act(y(eNOS),rpar(:,34)))*ymax(ROSec) - y(ROSec))/tau(ROSec);
...
dydt(NO) = (OR(act(y(eNOS),rpar(:,32)),act(y(Calcium),rpar(:,38)))*ymax(NO) - y(NO))/tau(NO); %rpar index fix from dydt(NO) = (OR(act(y(eNOS),rpar(:,31)),act(y(Calcium),rpar(:,38)))*ymax(NO) - y(NO))/tau(NO); %%%(AND(rpar(:,33),act(y(eNOS),rpar(:,33)),act(y(Calcium),rpar(:,33)))*ymax(NO) - y(NO))/tau(NO); % 
...
dydt(ActinR) = (act(y(MLC),rpar(:,47))*ymax(ActinR) - y(ActinR))/tau(ActinR); %switched for readability, does not impact results
