...
%         CI_diameter_ub(1,:) = mean(s_FC(:,:,38)) + std(s_FC(:,:,38))*1.96; % upper-bound for diameter predicted
%         CI_diameter_lb(1,:) = mean(s_FC(:,:,38)) - std(s_FC(:,:,38))*1.96;
% 
%         CI_number_ub(1,:) = mean(s_FC(:,:,37)) + std(s_FC(:,:,37))*1.96; % upper-bound for number predicted
%         CI_number_lb(1,:) = mean(s_FC(:,:,37)) - std(s_FC(:,:,37))*1.96;
...
%%
     ...
     b.CData(1,:) = [0 0 1];
     b.CData(2,:) = [0 0 0];
%    hold on; subplot(2,1,1); er = errorbar([2:7], [mean(s_FC([1:Nn],:,37))], (CI_number_lb-CI_number_ub)); er.Color = [0 0 0];          er.LineStyle = 'none';  
...
%    hold on; subplot(2,1,2); er = errorbar([2:7], [mean(s_FC([1:Nn],:,38))], (CI_diameter_lb - CI_diameter_ub)); er.Color = [0 0 0];  er.LineStyle = 'none';  
...
figure(53); subplot(2,1,1); heatmap(real(s_FD_Ym([37],[1:36],1)), 'Colormap', jet); ax = gca; ax.XData = params{4}([1:36]); ax.YData = params{4}([37])  ; ax.FontSize = 12; 
figure(54); subplot(2,1,1); heatmap(real(s_FD_Ym([38],[1:36],1)), 'Colormap', jet); ax = gca; ax.XData = params{4}([1:36]); ax.YData = params{4}([38])  ; ax.FontSize = 12; 
...
figure(53); subplot(2,1,2); heatmap(real(s_FD_W([37],[2:end],1)), 'Colormap', jet); ax = gca; ax.XData = params{5}([2:end]);  ax.YData = params{4}([37]);   ax.FontSize = 12; 

figure(54); subplot(2,1,2); heatmap(real(s_FD_W([38],[2:end],1)), 'Colormap', jet); ax = gca; ax.XData = params{5}([2:end]);  ax.YData = params{4}([38]);  ax.FontSize = 12; 
...
if task == 4
    ...
    for Nstep = 1:Nn
        ...
        YstepP(Nstep,:,:) = real(Y10);
        fprintf('run %i finished\n', Nstep)

    end

    Ymean10(1,:,:) = mean(YstepP([1:Nn],:,:));
...
figure(5);
    subplot(3,2,1);
    bar((Ymean(end,[2:36]) - Ymean(1,[2:36])), 'k'); title('No intervention'); 
    xticks([1:35]); xticklabels(params{4}([2:36])); ylabel('Change relative to baseline')
    ...
    figure(5);
    subplot(3,2,3);
    bar((Y4(end,[2:36]) - Y4(1,[2:36])), 'k'); title('Glucose intervention at 4 weeks'); 
    xticks([1:35]); xticklabels(params{4}([2:36])); ylabel('Change relative to baseline')
    ...
    figure(5);
    subplot(3,2,5);
    bar((Ymean10(end,[2:36]) - Ymean10(1,[2:36])), 'k'); title('Glucose intervention at 10 weeks'); 
    xticks([1:35]); xticklabels(params{4}([2:36])); ylabel('Change relative to baseline')
