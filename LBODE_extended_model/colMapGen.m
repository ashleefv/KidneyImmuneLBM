...
function [colMap] = colMapGen(topCol,botCol,numCol,fraction,varargin)
    ...
    % fraction is 1 or -0.5 or 0.5 depending on if you want to center at 0
    % or only use the bottom or top half of the colormap
    ...
    addRequired(p,'fraction',@isnumeric); % required function input
    addParameter(p,'midCol',[1 1 1],@isnumeric); % varargin input  
    parse(p,topCol,botCol,numCol,fraction,varargin{:}); % parse inputs
    ...
    fraction = p.Results.fraction; % define function variable from inputs
